	...
	class StbBlock {
		...
		private function getCssClasses( $atts = null ) {
			...
			if ( is_array( $atts ) ) {
				...
				if ( $atts['collapsing'] === 'default' ) {
					$collapsing = ( $settings['collapsing'] === 'true' );
				} else {
					$collapsing = ( $atts['collapsing'] === 'true' );
				}
				...
				if ( ! empty( $atts['caption'] ) ) {
					array_push( $cntClasses, 'stb-caption' );
					if ( $collapsing ) {
					...
					array_push( $cntClasses, 'stb-no-caption' );
				}

				// Image Size
				$value = ( ( $settings['bigImg'] === 'true' ) ? 'stb-image-big' : 'stb-image-small' );
				if ( ! empty( $atts['big'] ) ) {
					if ( $atts['big'] != $settings['bigImg'] ) {
						$value = ( ( $atts['big'] == 'true' ) ? 'stb-image-big' : 'stb-image-small' );
					}
				}
				if ( ( $atts['image'] === 'null' ) || $settings['showImg'] !== 'true' ) {
					$value = 'stb-image-none';
				}
				array_push( $cntClasses, $value );
				...
				// Rounded Corners
				if ( $settings['rounded_corners'] === 'true' ) {
					array_push( $cntClasses, 'stb-corners' );
				}

				// Shadow
				$value = '';
				if ( $settings['box_shadow'] === 'true' ) {
					$value = 'stb-shadow';
				}
				...
				if ( $atts['caption'] == '' ) {
					if ( $side ) {
						array_push( $cntClasses, 'stb-side' );
					} else {
						array_push( $cntClasses, 'stb-side-none' );
					}
				}
			}
		...
		private function getCssStyles( $atts = null, $idNum = 0 ) {
			...
			if ( is_array( $atts ) ) {
				...
				if ( ! empty( $atts['image'] ) && ( $atts['image'] != 'null' ) ) {
					if ( empty( $atts['caption'] ) ) {
						$iconImg = "<aside class='stb-icon'><img src='{$atts['image']}'></aside>";
					} else {
						$iconImg = "<aside class='stb-caption-icon'><img src='{$atts['image']}'></aside>";
					}
				} else {
					if ( empty( $atts['caption'] ) ) {
						$iconImg = "<aside class='stb-icon'><img src='{$sImg}'></aside>";
					} else {
						$iconImg = "<aside class='stb-caption-icon'><img src='{$sImg}'></aside>";
					}
				}
				...
				if ( ! empty( $atts['cbgcolor'] ) || ! empty( $atts['cbgcolorto'] ) ) {
					...
					if ( empty( $atts['caption'] ) ) {
						$cnt = "background-image: linear-gradient(#{$c1} 30%, #{$c2} 90%);";
					} else {
						$cap = "background-image: linear-gradient(#{$c1} 30%, #{$c2} 90%);";
					}
				}
				if ( ! empty( $atts['ccolor'] ) ) {
					$cap .= "color: #{$atts['ccolor']};";
				}
		...
		private function getJsStyles( $atts = null, $idNum = 0 ) {
			...
			*/
			...
			if ( ! empty( $atts['caption'] ) ) {
				...
				if ( ! empty( $atts['collapsed'] ) ) {
					$stbCaption .= ", collapsed: " . $atts['collapsed'];
				}
				if ( $atts['collapsing'] != 'default' ) {
					$stbCaption .= ", collapsing: " . $atts['collapsing'];
				}
				if ( ! empty( $atts['ccolor'] ) ) {
					$stbCaption .= ', fontColor: "#' . $atts['ccolor'] . '"';
				}
				if ( ! empty( $atts['cbgcolor'] ) ) {
					$stbCaption .= ', color: "#' . $atts['cbgcolor'] . '"';
				}
				if ( ! empty( $atts['cbgcolorto'] ) ) {
					$stbCaption .= ', colorTo: "#' . $atts['cbgcolorto'] . '"';
				}
			}
			if ( ! empty( $atts['bwidth'] ) ) {
				$stbBorder = 'width: ' . $atts['bwidth'];
				if ( ! empty( $atts['bcolor'] ) ) {
					$stbBorder .= ', color: "#' . $atts['bcolor'] . '"';
				}
			}
			if ( ! empty( $atts['shadow'] ) ) {
				$stbShadow = 'enabled: ' . $atts['shadow'];
			}
			if ( strlen( $atts['mtop'] ) > 0 ) {
				$stbOpts .= ", mtop: " . $atts['mtop'];
			}
			if ( strlen( $atts['mright'] ) > 0 ) {
				$stbOpts .= ", mright: " . $atts['mright'];
			}
			if ( strlen( $atts['mbottom'] ) > 0 ) {
				$stbOpts .= ", mbottom: " . $atts['mbottom'];
			}
			if ( strlen( $atts['mleft'] ) > 0 ) {
				$stbOpts .= ", mleft: " . $atts['mleft'];
			}
			if ( ! empty( $atts['color'] ) ) {
				$stbOpts .= ', fontColor: "#' . $atts['color'] . '"';
			}
			if ( ! empty( $atts['bgcolor'] ) ) {
				$stbOpts .= ', color: "#' . $atts['bgcolor'] . '"';
			}
			if ( ! empty( $atts['bgcolorto'] ) ) {
				$stbOpts .= ', colorTo: "#' . $atts['bgcolorto'] . '"';
			}
			if ( ! empty( $atts['direction'] ) ) {
				$stbOpts .= ', direction: "' . $atts['direction'] . '"';
			}
			if ( ! empty( $atts['image'] ) ) {
				$stbOpts .= ', image: ' . ( ( $atts['image'] == 'null' ) ? 'null' : '"' . $atts['image'] . '"' );
			} elseif ( $settings['showImg'] !== 'true' ) {
				$stbOpts .= ', image: null';
			}

			if ( ! empty( $stbCaption ) ) {
				$stbData = "caption: {{$stbCaption}}";
			}
			if ( ! empty( $stbBorder ) ) {
				if ( ! empty( $stbData ) ) {
					$stbData .= ", border: {{$stbBorder}}";
				} else {
					$stbData = "border: {{$stbBorder}}";
				}
			}
			if ( ! empty( $stbOpts ) ) {
				if ( ! empty( $stbData ) ) {
					...
					$stbData = substr_replace( $stbOpts, '', 0, 2 );
				}
			}
			if ( ! empty( $stbShadow ) ) {
				if ( ! empty( $stbData ) ) {
					$stbData .= ", shadow: {{$stbShadow}}";
				} else {
					$stbData = "shadow: {{$stbShadow}}";
				}
			}
		...
		public function getWidgetBox( $data = null ) {
			...
			if ( is_null( $boxData ) ) {
				...
				$atts = shortcode_atts( $defaults, $boxData['atts'] );
			}
			...
			if ( isset( $atts['mode'] ) ) {
				...
				$mode = self::getMode( '', $settings['mode'] );
			}
			$atts['mode'] = $mode;
			...
			if ( $mode == 'js' ) {
				...
				if ( $caption == '' ) {
					...
					$out = array(
						'before'      => $cntStart,
						'after'       => "</div>{$cntEnd}",
						'beforeTitle' => "<div id='stb-caption-box-$idNum' class='stb-$id-caption_box stb_caption stb-caption-box' {$cssBlock['cap']}>{$cssBlock['icon']}<span>",
						'afterTitle'  => "</span>{$cssBlock['tool']}</div><div id='stb-body-box-$idNum' class='stb-$id-body_box stb_body stb-body-box' {$cssBlock['body']}>"
					);
				}
			}
		...
		private function buildBlock( $data ) {
			...
			if ( $mode == 'js' ) {
				...
				if ( $caption === '' ) {
					...
					if ( in_array( $id, $stbClasses ) && $id !== 'grey' ) {
						return $cssBlock['floatStart'] . $cntStart . "<div id='stb-caption-box-$idNum' class='stb-$id-caption_box stb_caption stb-caption-box' {$cssBlock['cap']}>{$cssBlock['icon']}<span>{$caption}</span>{$cssBlock['tool']}</div><div id='stb-body-box-$idNum' class='stb-$id-body_box stb_body stb-body-box' {$cssBlock['body']}>" . do_shortcode( $content ) . "</div>" . $cntEnd . $cssBlock['floatEnd'];
					} elseif ( in_array( $id, $stbClasses ) && $id === 'grey' ) {
						return $cssBlock['floatStart'] . $cntStart . "<div id='stb-caption-box-$idNum' class='stb-$id-caption_box stb_caption stb-caption-box' {$cssBlock['cap']}>{$cssBlock['icon']}<span>{$caption}</span>{$cssBlock['tool']}</div><div id='stb-body-box-$idNum' class='stb-$id-body_box stb_body stb-body-box' {$cssBlock['body']}>$content</div>" . $cntEnd . $cssBlock['floatEnd'];
					} else {
						return do_shortcode( $content );
					}
				}
