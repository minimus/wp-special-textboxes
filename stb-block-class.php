  ...
  class StbBlock {
    ...
    private function extendedStyleLogic($atts = null, $idNum = 0) {
      ...
      if ( is_array($atts) ) {
        ...
        $iconImg = "<aside style='float: {(($direction === 'ltr')?'left':'right')}; width: 25px;'><img src='{$image}'></aside>";
          ...
          if ($atts['image'] === '') {
            if ($needResizing & ($settings['showImg'] === 'true')) {
              if (!in_array($atts['id'], array('custom', 'grey'))) {
                $image = ( $atts['big'] === 'true' ) ? STB_URL.'images/'."{$atts['id']}-b.png" : STB_URL.'images/'."{$atts['id']}.png";
